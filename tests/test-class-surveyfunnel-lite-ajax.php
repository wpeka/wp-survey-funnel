...
class Test_Surveyfunnel_Lite_Ajax extends WP_Ajax_UnitTestCase {
	...
	/**
	 * Test for surveyfunel lite export csv.
	 */
	...
	/**
	 * Test for surveyfunnel lite get posts pages
	 *
	 * @dataProvider flagProvider
	 * @param bool $flag Flag.
	 */
	public function test_surveyfunnel_lite_get_posts_pages( $flag ) {
		// become administrator.
		$this->_setRole( 'administrator' );

		$_POST['action']   = 'surveyfunnel_lite_get_posts_pages';
		$_POST['security'] = wp_create_nonce( 'surveyfunnel-lite-security' );
		...
		$repsonse = json_decode( $this->_last_response );
		$this->assertTrue( $repsonse->success );
		$this->assertSame( 'Posts', $repsonse->data[0]->label );
		$this->assertSame( 'Pages', $repsonse->data[1]->label );

	}
	/**
	 * Data provider function for surveyfunnel lite get post pages test cases.
	 */
